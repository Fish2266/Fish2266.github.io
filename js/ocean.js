...
vec3 floatLocal(vec3 p, int i) {
  vec3 q = p - uFloatA[i].xyz;
  float cr = cos(uFloatB[i].x), sr = sin(uFloatB[i].x);
  float cp = cos(uFloatB[i].y), sp = sin(uFloatB[i].y);
  q = vec3(q.x * cr + q.y * sr, -q.x * sr + q.y * cr, q.z);
  q = vec3(q.x, q.y * cp + q.z * sp, -q.y * sp + q.z * cp);
  return q;
}

float sdFloat(vec3 p, int i) {
  vec3 q = floatLocal(p, i);
  vec2 c = vec2(length(q.xz) - uFloatA[i].w, q.y);
  return length(c) - uFloatB[i].z;
}

// Distance to one solid by id: rings are 0..7, creatures 8 and up.
float sdSolid(vec3 p, int id) {
  return id < 8 ? sdFloat(p, id) : sdCreature(p, id - 8);
}

// Nearest solid among this ray's candidates.
// Bounding radii, shared with the per-ray candidate test below.
float floatBound(int i) { return uFloatA[i].w + uFloatB[i].z * 2.0 + 0.1; }
...
float solidsMap(vec3 p, uint mask, out int which) {
  ...
  for (int i = 0; i < MAX_FLOATS; i++) {
    if (i >= uFloatN) break;
    if ((mask & (1u << uint(i))) == 0u) continue;
    float bd = length(p - uFloatA[i].xyz) - floatBound(i);
    float d = bd > 0.05 ? bd : sdFloat(p, i);
...
vec3 solidNormal(vec3 p, int id) {
  ...
  return normalize(vec3(
    sdSolid(p + e.xyy, id) - sdSolid(p - e.xyy, id),
    sdSolid(p + e.yxy, id) - sdSolid(p - e.yxy, id),
    sdSolid(p + e.yyx, id) - sdSolid(p - e.yyx, id)));
}
...
float traceSolids(vec3 ro, vec3 rd, float tMax, out int hitIdx) {
  ...
  for (int i = 0; i < MAX_FLOATS; i++) {
    ...
    vec3 oc = ro - uFloatA[i].xyz;
    float br = floatBound(i);
    float b = dot(oc, rd);
...
vec3 shadeSolid(vec3 ro, vec3 rd, vec3 p, vec3 n, int id, float dist, float waterH) {
  return id < 8 ? shadeFloat(ro, rd, p, n, id, dist, waterH)
                : shadeCreature(ro, rd, p, n, id - 8, dist, waterH);
}
...
export class Ocean {
  constructor(canvas) {
    ...
    // Quality ladder, best first. Resolution goes before wave detail, and
    // detail never reaches zero -- a glass-smooth sea reads as broken rather
    // than as a lower setting.
    this.baseScale = Math.min(window.devicePixelRatio || 1, 1.25);
    this.LADDER = [
      { mul: 1.00, detail: 1.00 },
      { mul: 0.82, detail: 1.00 },
      { mul: 0.68, detail: 0.80 },
      { mul: 0.55, detail: 0.55 },
    ];
    this.qual = 0;
  ...
  poll() {
    ...
    for (const n of ['uRes','uTime','uCamPos','uCamFwd','uCamRight','uCamUp',
                     'uTanHalf','uAspect','uSunDir','uDetail','uWake','uWakeN',
                     'uFloatA','uFloatB','uFloatC','uFloatN',
  ...
  setQuality(level) {
    const q = Math.max(0, Math.min(this.LADDER.length - 1, level));
    if (q === this.qual) return;
    this.qual = q;
    const step = this.LADDER[q];
    this.detail = step.detail;
    this.scale = Math.max(0.5, this.baseScale * step.mul);
  ...
  render(time, dt, wakes, floats, creatures) {
    if (!this.ok) return;
    if (!this.ready && !this.poll()) return;   // shader still compiling
    const gl = this.gl, u = this.u;

    // Hold the frame rate on weaker GPUs, and climb back when there is room.
    //
    // Judged on real elapsed time, not the scene's dt: that is clamped to
    // 0.05s, which is already past the "slow" threshold, so a throttled tab
    // looked exactly like an overloaded GPU and quality fell permanently.
    if (this.adapt) {
      const now = performance.now();
      const gap = this.lastFrame ? (now - this.lastFrame) / 1000 : 0;
      this.lastFrame = now;
      // A gap beyond 0.2s is a stall or a throttle, not a frame rate.
      if (!document.hidden && gap > 0 && gap < 0.2) {
        if (gap > 0.026) { this.slow++; this.fast = 0; } else { this.fast++; this.slow = 0; }
        if (this.slow > 45) { this.setQuality(this.qual + 1); this.slow = 0; }
        else if (this.fast > 240) { this.setQuality(this.qual - 1); this.fast = 0; }
      }
    }
    ...
    const n = Math.min(32, ((wakes && wakes.length) || 0) / 4 | 0);
    if (n > 0) {
      this.wakeBuf ||= new Float32Array(128);
      this.wakeBuf.set(wakes.slice(0, n * 4));
      gl.uniform4fv(u.uWake, this.wakeBuf);
    }
